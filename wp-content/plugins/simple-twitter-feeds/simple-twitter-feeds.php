<?php
/*
Plugin Name: Simple Twitter Feeds
Plugin URI: http://www.hohb.nl
...
class PI_SimpleTwitterFeeds extends WP_Widget{

	function PI_SimpleTwitterFeeds()  {
		$widget_ops = array('classname' => 'PI_SimpleTwitterFeeds', 'description' => 'Displays the most recent tweets from your Twitter Stream' );
		//$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'Not-required-this-time' );
		$this->WP_Widget('PI_SimpleTwitterFeeds', 'Simple Twitter Feeds', $widget_ops);
	...
	function register_stt_styles() {
		if(!is_admin()){
			wp_register_style( 'PI_stt_front', plugins_url( 'simple-twitter-feeds/css/stt.min.css' ) );
	...
	function PI_load_admin_scripts($hook) {
		...
		// Get global $wp_version - what version of WordPress is installed.
			...
			// Now load STT JS
		wp_enqueue_script('PI_stt_js', plugins_url( '/simple-twitter-feeds/js/sttAdmin.min.js' , dirname(__FILE__) ), array('jquery'));
	}
	...
	function widget($args, $instance){
		...
		if (!empty($PI_title))
					...
					<?php foreach($tweets as $t) : ?>
							<div class="large-4 small-12 columns">
                                <div class="tw-wrap">
									...
									<?php
                                        if ($showAvatar){
                                            echo '<img src="'.$t['image'].'" alt="Tweet Avatar" />';
                                        }
                                    ?>			
                                    </div>
			...
			<?php else : ?>
					<div>Waiting for Twitter... Once Twitter is ready they will display my Tweets again.</div>
			<?php endif; ?>
...
add_action( 'widgets_init', create_function('', 'return register_widget("PI_SimpleTwitterFeeds");') );
?>